...
Ve skupině Uhynulých máme `r length(perished)` hodnot, ve skupině přeživších `r length(survived)`.
...
histogram vykreslí. Aby histogram byl normalizovaný (součet výšek byl roven 1) 
a nabýval obdobných hodnot jako hustota, musí být parametr `freq` nastaven 
na `FALSE`.
...
Pro vygenerování n náhodných hodnot při daném rozdělení využijeme opět balíčku 
`EnvStats`. Zvolili jsme normální rozdělení, proto použijeme funkci `rnorm`. Jako
parametry zadáme počet generovaných hodnot, už dříve odhadnutý výběrový průměr a 
druhou odmocninu výběrového rozptylu (zde místo výběrové směrodatné odchylky).
